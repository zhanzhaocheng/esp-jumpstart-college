...
void esp_fill_mydata(void *buf, uint8_t* data,size_t len)
{
    ...
    int k=0;
    while (len > 0) {
        uint8_t word = (uint8_t)(2+k);
        uint32_t to_copy = sizeof(word);
        if(sizeof(word)>len){
             to_copy = len;
        }
...
void my_espnow_data_prepare(example_espnow_send_param_t *send_param)
{
    ...
    esp_fill_mydata(buf->payload,(uint8_t*)mydata, send_param->len - sizeof(example_espnow_data_t));
    ...
    while (xQueueReceive(s_example_espnow_queue, &evt, portMAX_DELAY) == pdTRUE) {
        switch (evt.id) {
            case EXAMPLE_ESPNOW_SEND_CB:
            {
                ...
                my_espnow_data_prepare(send_param);
            ...
            case EXAMPLE_ESPNOW_RECV_CB:
            {
                ...
                if (ret == EXAMPLE_ESPNOW_DATA_BROADCAST) {
                    ...
                    for(int i=0l;i<50;i++)
                    {
                        ESP_LOGI(TAG, "the %dth unicast data is : %d", i+1,  recv_cb->data[i]);
                    }
